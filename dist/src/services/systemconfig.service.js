 ...
 */
export async function getSystemConfigById(id) {
    ...
    switch (config.type) {
        ...
        case "JSON":
            try {
                ...
                throw new Error(`Invalid JSON value for system configuration: ${key}`);
            }
        case "DATE":
 ...
 */
export async function updateSystemConfig(id, data) {
    const config = await prisma.systemConfig.findUnique({
        where: {
            id,
        },
