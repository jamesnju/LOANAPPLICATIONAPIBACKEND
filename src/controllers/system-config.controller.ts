...
} from "../schemas/system-config.schema.js";

import {
  createSystemConfig as createConfig,
  getSystemConfigs,
  getSystemConfigById,
  updateSystemConfig as updateConfig,
  deleteSystemConfig as deleteConfig,
...
export async function createSystemConfig(
  ...
  try {
     ...
     */
    const data =
      createSystemConfigSchema.parse(req.body);
...
export async function getAllSystemConfigs(
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      message:
        "Unable to retrieve system configurations",
    });
  }
}
 ...
 */
export async function getOneSystemConfig(
  req: Request,
  res: Response
): Promise<void> {
  ...
  try {
    ...
    if (!id) {
      ...
      res.status(400).json({
        success: false,
        message:
          "System configuration ID is required",
      });
  ...
  try {
    ...
    if (
      ...
      res.status(409).json({
        success: false,
        message:
          "System configuration key already exists",
      });
...
export async function deleteSystemConfig(
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      message:
        "Unable to delete system configuration",
    });
  }
}
